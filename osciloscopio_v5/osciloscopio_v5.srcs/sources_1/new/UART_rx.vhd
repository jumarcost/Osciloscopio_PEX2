...
architecture Behavioral of UART_rx is
    ...
    signal rx_state,rx_state_ant: rx_states_t := IDLE;
...
-- four states (IDLE, START, DATA, STOP). See inline comments for more details.
    ...
    begin
        if rising_edge(clk) then
            rx_state_ant <= rx_state;
        end if;
    end process;
    
    
    rx_ready <= '1' when ((rx_state_ant = STOP and rx_state = IDLE)or (rx_state_ant = IDLE and rx_state = STOP)) else '0';
